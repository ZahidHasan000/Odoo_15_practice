...
class LcManagement(models.Model):
    ...
    total_freight_amount = fields.Monetary(currency_field="currency_id",
        default=0.0, string="Freight Amount", tracking=True)
    ...
    bill_of_leading_no = fields.Char('Bill of Leading no.')
    bill_of_leading_date = fields.Date('B/L-Date.')
    bill_of_entries_no = fields.Char('Bill of Entries no.')
    bill_of_entries_date = fields.Date('B/E-Date.')
    ...
    c_and_f_date = fields.Date(string="Date")
            ...
            if rec.c_and_f_bill_number:
                ...
                if not rec.c_and_f_ids:
                    ...
                    move = rec.env['account.move'].sudo().create({
                        ...
                        'ref': reference,
                        'date': rec.c_and_f_date,
                        'invoice_date': rec.c_and_f_date,
                    ...
                    if move:
                        ...
                        for item in rec.c_and_f_ids.filtered(lambda line: line.is_landed_cost):
                            rec.env['landed.cost.item'].sudo().create({
                                'lc_id': rec.id,
                                'account_id': item.account_id.id,
                                'currency_id': rec.env.company.currency_id.id,
                                'label': 'C&F Bill For ' + rec.lc_number,
                                'landed_cost_amount': item.amount,
                            })
    ...
    def apply_landed_cost(self):
        for rec in self:
            if any(val['locked_item'] == False for val in rec.landed_cost_line):
                ...
                month = datetime.strptime(str(landed_cost.date), '%Y-%m-%d').strftime("%B")
                year = landed_cost.date.year
                rec.landed_cost_month = str(month) + '-' + str(year)
    ...
    landed_cost_month = fields.Char(string="Month of Landed cost")

    # def get_date_year(self):
    #     for rec in self:
    #         landed_cost = rec.env['stock.landed.cost'].sudo().search([('id', '=', 2)])
    #         month = datetime.strptime(str(landed_cost.date), '%Y-%m-%d').strftime("%B")
    #         year = landed_cost.date.year
    #
    #         rec.landed_cost_month = str(month) + '-' + str(year)
