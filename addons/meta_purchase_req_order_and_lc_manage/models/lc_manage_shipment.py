...
class LcManageShipmentInherit(models.Model):
    ...
    lc_shipment_bank_rate = fields.Float('Bank Rate', default=1.00, digits=(16, 12))
        ...
        for rec in self:
            ...
            if rec.lc_shipment_charge_account and rec.lc_shipment_pay_account and not rec.lc_shipment_journal:
                ...
                if journal:
                    ...
                    freight_product = rec.env['product.product'].sudo().search([('name', '=', 'Import Freight')])
                    rec.env['landed.cost.item'].sudo().create({
                        'lc_id': rec.id,
                        'account_id': freight_product.property_account_expense_id.id,
                        'currency_id': rec.env.company.currency_id.id,
                        'label': 'Import Freight For '+ rec.lc_number,
                        'landed_cost_amount': rec.lc_shipment_bank_rate * rec.total_freight_amount,
                    })