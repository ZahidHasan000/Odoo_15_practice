...
class LcManageVatAitInherit(models.Model):
    ...
    vat_ait_apply = fields.Boolean('Apply Vat Ait', default=False)
    ...
    def create_lc_vat_ait_journal(self):
        for rec in self:
            if rec.vat_ait_apply:
                raise UserError(_('Already Vat and Ait Applied Please check C&F line'))
            else:
                vat_product = rec.env['product.product'].sudo().search([('name', '=', 'VAT on Import')])
                ait_product = rec.env['product.product'].sudo().search([('name', '=', 'AIT on Import')])

                vat_amount = rec.lc_manage_vat_ait_currency_rate * rec.lc_manage_vat_import
                ait_amount = rec.lc_manage_vat_ait_currency_rate * rec.lc_manage_ait_import

                vat_c_and_f = rec.env['lc.candf.data'].sudo().create({
                    'lc_manage_id': rec.id,
                    'product_id': vat_product.id,
                    'account_id': rec.lc_manage_vat_account.id if rec.lc_manage_vat_account else vat_product.property_account_expense_id.id,
                    'amount': vat_amount if vat_amount > 0.00 else 0.00,
                    'is_landed_cost': False,
                })

                ait_c_and_f = rec.env['lc.candf.data'].sudo().create({
                    'lc_manage_id': rec.id,
                    'product_id': ait_product.id,
                    'account_id': rec.lc_manage_ait_account.id if rec.lc_manage_ait_account else ait_product.property_account_expense_id.id,
                    'amount': ait_amount if ait_amount > 0.00 else 0.00,
                    'is_landed_cost': False,
                })

                rec.vat_ait_apply = True
