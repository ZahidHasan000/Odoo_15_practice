        ...
        for rec in self:
            ...
            for item in rec.line_ids.filtered(lambda l: l.product_qty - (l.reserved_qty + l.procurement_qty) > l.qty_available):
                ...
                line_values = {
                    ...
                    'demand_qty': item.product_qty - item.procurement_qty,
                    'pr_request_line_id': item.id,
                    'qty_ordered': 0.00,
                    'product_uom_id': item.product_uom_id.id,
                    'price_unit': item.average_cost,
    ...
    def get_total_current_stock(self):
        for rec in self:
            if rec.product_id:
                ...
                for loc in location.filtered(lambda item: item.usage == 'internal'):
                ...
                for quant in stock_quant:
                    total_quantity.append(quant.available_quantity)
                print(total_quantity)
                rec['qty_available'] = sum(total_quantity)
            else:
                rec['qty_available'] = 0.00

    qty_available = fields.Float(compute="get_total_current_stock", string="Available Quantity")
...
class TenderLineInherit(models.Model):
    ...
    demand_qty = fields.Float('Demand Qty')

    # @api.onchange('product_qty')
    # def check_pr_quantity(self):
    #     for rec in self:
    #         if rec.pr_request_line_id:
    #             need_qty = rec.pr_request_line_id.product_qty
    #             quantity = rec.pr_request_line_id.product_qty
    #             if rec.product_qty > need_qty:
    #                 # rec.pr_request_line_id.procurement_qty = rec.product_qty
    #
    #                 raise UserError(_('You enter more then the demand qty please check first !'))

    # def write(self, vals):
    #     res = super(TenderLineInherit, self).write(vals)
    #     # for rec in self:
    #     if vals.get("product_qty"):
    #         print('hit here')
    #         self.product_qty = self.pr_request_line_id.product_qty
    #         self.pr_request_line_id.procurement_qty = vals.get("product_qty")
    #
    #     return res
...
class TenderInherit(models.Model):
    ...
    def create(self, vals_list):
        ...
        if res.purchase_request:
            ...
            for item in res.line_ids:
                if item.pr_request_line_id:
                    if item.product_qty > item.demand_qty:
                        raise UserError(_('You enter more then the demand qty please check first !'))
                    else:
    ...
    def write(self, vals):
        res = super(TenderInherit, self).write(vals)
        ...
        if vals.get('line_ids'):
            if self.purchase_request:
                for item in self.line_ids:
                    if item.pr_request_line_id:
                        # print("Hit 2")
                        need_qty = item.pr_request_line_id.product_qty
                        if item.product_qty > item.demand_qty:
                            raise UserError(_('You enter more then the demand qty please check first !'))
                        # else:
                        #     needed = item.pr_request_line_id.product_qty - item.pr_request_line_id.procurement_qty
                        #     if item.product_qty > needed:
                        #         raise UserError(_('You enter more then the demand qty please check first !'))
                        #     else:
                        #         item.pr_request_line_id.procurement_qty += item.product_qty
    ...
    def add_product(self):
        return {
                'name': _('Purchase Request Product'),
                'view_mode': 'form',
                'res_model': 'product.add.wizard',
                'view_id': self.env.ref('meta_easy_way_create_and_view.product_add_wizard_form').id,
                'type': 'ir.actions.act_window',
                'res_id': False,
                'target': 'new',
                'context': {
