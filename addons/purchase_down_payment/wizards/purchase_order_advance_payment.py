...
class PurchaseOrderAdvancePayment(models.TransientModel):
    ...
    @api.model
    def _default_has_down_payment_message_show(self):
        """Function to check the Purchase order has down payment or not"""
        if self._context.get('active_model') == 'purchase.order' and self._context.get('active_id', False):
            ...
            down_total = []
            for item in purchase_order.order_line.filtered(lambda purchase_order_line: purchase_order_line.is_downpayment and purchase_order_line.qty_invoiced > 0.00):
                down_total.append(item.price_unit)

            if sum(down_total) >= purchase_order.amount_total:
                return True
            else:
                return False

    show_message = fields.Boolean('Show Message', default=_default_has_down_payment_message_show)
    ...
    deduct_down_payments = fields.Boolean('Without Deduct down payments', default=False,
