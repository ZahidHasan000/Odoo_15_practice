...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    analytic_account = fields.Many2one('account.analytic.account', string="Project Number")
    ...
    def default_get(self, fields):
        ...
        if active_model == "purchase.request.line":
            ...
            print('partner partner:', request_ids[0])
            res['request_id'] = request_ids[0]

            pr_request = self.env["purchase.request"].browse(request_ids[0])
            res['analytic_account'] = pr_request.project_number.id
            res['customer'] = pr_request.customer_name.id
    ...
    def _prepare_purchase_order(self, picking_type, group_id, company, origin):
        ...
        data = {
            ...
            "customer_name": self.customer.id,
            "customer_reference": self.analytic_account.id,
            "purchase_request": self.request_id.id,
    ...
    def make_purchase_order(self):
        ...
        purchase_order_request = self.env['purchase.order'].sudo().search([('id', '=', purchase.id)])
        if purchase_order_request:
            purchase_order_request.purchase_request = self.request_id.id
